...
const makeNgrams = (values, length) => {
  ...
  for (let i = 0; i < valuesLen - length + 1; i++) {
 ...
 * @return {array<array<Number>>} Signatures which represent shingles in compact form
 */
...
const calculateSimilarities = (signatures) => {
  ...
  for (let i = 0; i < signatures.length; i++) {
    ...
    for (let j = i + 1; j < signatures.length; j++) {
      ...
      const estimate = intersectionSize / firstSignature.length
 ...
 * @param {Array<string>} emailIds Array with ids of all available emails
 * @param {Function} Function which will fetch email by its id. May be called more than once
 * @return {object} Object with 'ratings' and 'averageRating' fields.
 */
const calculateRatings = (emailIds, getEmailById) => {
  const shingles = emailIds.map(emailId => {
    const email = getEmailById(emailId)
    return makeShingles(email.topic + email.content)
  })
...
if (process.env.NODE_ENV === 'test') {
  module.exports.arrayIntersectionSize = arrayIntersectionSize
  module.exports.makeNgrams = makeNgrams
  module.exports.makeWordNgrams = makeWordNgrams
  module.exports.makeShingles = makeShingles
  module.exports._generateSignatures = _generateSignatures
  module.exports.calculateSimilarities = calculateSimilarities
  module.exports.calculateRatingsActual = calculateRatingsActual
}
